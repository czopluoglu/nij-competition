...
ridge <- cv.glmnet(x = as.matrix(train[,2:645]),
                   y = out1,
                   alpha = 0)
...
pred <- predict(ridge.fit,as.matrix(test[,2:645]))
...
lasso <-  cv.glmnet(x = as.matrix(train[,2:645]),
                    y = out1,
                    alpha = 1)
...
pred <- predict(lasso.fit,as.matrix(test[,2:645]))
pred <- exp(pred)/(1+exp(pred))
...
elastic.fit <- glmnet(x = as.matrix(train[,2:645]), 
                      y = out1, 
                      alpha  = 0.6, 
                      lambda = 0.005666559,
                      family = "binomial")

pred <- predict(elastic.fit,as.matrix(test[,2:645]))
pred <- exp(pred)/(1+exp(pred))
...
write.csv(outcome, 
          here('data','glmnet_test_model1.csv'),
          row.names = FALSE)


pred <- predict(elastic.fit,as.matrix(final_test2[,2:645]))
pred <- exp(pred)/(1+exp(pred))
...
write.csv(outcome2, 
          here('data','glmnet_comptetition_test_model1.csv'),
          row.names = FALSE)
...
main.effects <- colnames(train[,2:645])[sig[2:length(sig)]]
################################################################################
################################################################################
################################################################################
#
...
write.csv(outcome, 
          here('data','glmnet_test_model2.csv'),
          row.names = FALSE)



final_test2$out <- rep(1,7807)
  
f <- as.formula(out ~ .*.)

X2 <- model.matrix(f, final_test2[,c(main.effects,'out')])[, -1]
...
write.csv(outcome2, 
          here('data','glmnet_comptetition_test_model2.csv'),
          row.names = FALSE)
